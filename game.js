(() => {
  ...
  const upgradeDefs = [
    { key:'fireRate', name:'Rapid Fire', desc:'Shoot more often.', base:100, step:125, max:6 },
    { key:'bulletSpeed', name:'Hyper Bolts', desc:'Shots travel faster.', base:100, step:150, max:5 },
    { key:'spread', name:'Multi-Shot', desc:'Adds extra side shots.', base:200, step:250, max:3 },
    { key:'power', name:'Plasma Power', desc:'Stronger shots can punch through more enemies.', base:250, step:300, max:4 }
  ];
  ...
  function start() {
    reset(); playing=true;
    ui.start.classList.remove('shown'); ui.over.classList.remove('shown'); ui.upgrades.classList.remove('shown');
    last=performance.now();
  }

  function gameOver() {
    playing=false; shopping=false;
    ui.final.textContent=Math.floor(score); ui.initials.value='';
  ...
  function fire() {
    ...
    const speed = 560 + upgrades.bulletSpeed * 85;
    const damage = 1 + upgrades.power;
    const addShot = (vx=0, offset=0) => shots.push({x:player.x+offset,y:player.y-24,w:4,h:14,vx,vy:-speed,damage,pierce:Math.max(0,upgrades.power-1)});
    addShot();
    if(upgrades.spread>=1){ addShot(-80,-8); addShot(80,8); }
    if(upgrades.spread>=2){ addShot(-145,-14); addShot(145,14); }
    if(upgrades.spread>=3){ addShot(-210,-18); addShot(210,18); }
    fireCooldown=Math.max(.065,.22-upgrades.fireRate*.027);
  }
  ...
  function renderUpgrades() {
    ...
    for(const def of upgradeDefs){
      ...
      const c=document.createElement('p'); c.className='cost'; c.textContent=maxed?'MAXED':`COST: ${cost} PTS`;
      const b=document.createElement('button'); b.type='button'; b.textContent=maxed?'MAXED OUT':'BUY UPGRADE';
      b.disabled=maxed||score<cost;
      b.addEventListener('click',()=>{
        const liveCost=upgradeCost(def);
        if(upgrades[def.key]>=def.max||score<liveCost) return;
        score-=liveCost; upgrades[def.key]++; hud(); renderUpgrades();
      });
      card.append(h,p,c,b); ui.upgradeChoices.appendChild(card);
    }
  }
  ...
  function update(dt) {
    ...
    for(const s of shots){ s.y+=s.vy*dt; s.x+=(s.vx||0)*dt; }
    ...
    for(let n=shots.length-1;n>=0;n--){
      ...
      if(bonus&&!used&&overlap(shot,bonus)){
        score+=1000; burst(bonus.x,bonus.y,'#ffd966',28); bonus=null;
        if(shot.pierce>0) shot.pierce--; else used=true;
        hud();
      }
      if(!used){
        for(const i of invaders) if(i.alive&&overlap(shot,i)){
          i.hp-=shot.damage; burst(shot.x,shot.y,i.color,6);
          if(i.hp<=0){ i.alive=false; score+=i.points; burst(i.x,i.y,i.color,i.kind==='brute'?20:12); hud(); }
          if(shot.pierce>0) shot.pierce--; else used=true;
          break;
        }
      }
    ...
    if(alive.length){
      ...
      for(const i of alive){
        ...
        minX=Math.min(minX,i.x-i.w/2); maxX=Math.max(maxX,i.x+i.w/2);
      }
      if((direction>0&&maxX>W-24)||(direction<0&&minX<24)){
        direction*=-1; for(const i of alive) i.y+=18;
      }
      if(alive.some(i=>i.y+i.h/2>=player.y-12)){ lives=0; hud(); gameOver(); }
    } else if(invaders.length){
      invaders.length=0; openUpgradeBay();
    }
  ...
  function drawPlayer(){
    ...
    if(upgrades.spread>0){ rect(x-27,y+2,6,10,'#ffd966'); rect(x+21,y+2,6,10,'#ffd966'); }
  ...
  function drawInvader(i,t){
    ...
    if(i.hp<i.maxHp) rect(i.x-10,i.y+i.h/2+4,20*(i.hp/i.maxHp),2,'#fff');
  }
  ...
  function saveScore(){
    ...
    localStorage.setItem('pixel-invaders-scores',JSON.stringify(scores.slice(0,20))); renderScores();
    ui.saveBtn.textContent='SAVED!'; setTimeout(()=>ui.saveBtn.textContent='SAVE SCORE',900);
  }
  ...
  async function toggleFullscreen(){
    try{
      ...
      ui.fullscreenBtn.textContent=document.body.classList.contains('fullscreen-mode')?'EXIT FULL SCREEN':'FULL SCREEN';
    }
  }
